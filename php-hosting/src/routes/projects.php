...
function handleProjectsCreate(): void {
    ...
    getDb()->prepare(
        'INSERT INTO projects (id, name, description, owner_user_id, api_key_hash, api_key_last4, plan_id, idempotency_key, created_at)
         VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?)'
    )->execute([$id, $name, $description, $session['sub'], tokenHash($apiKey), substr($apiKey, -4), $idempotencyKey !== '' ? $idempotencyKey : null, now()]);
